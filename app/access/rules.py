...
def servers_needing_self_grant(user):
    ...
    Архивные серверы исключены. Выдавать на них доступ незачем, а цена была не
    только в бессмысленности: каждый сервер — отдельный запрос опроса, и на
    боевом парке в очередь попадало 194 архивных против 12 настоящих. Если
    сервер вернут в работу, следующий проход подхватит его сам — остаток
    считается заново каждый раз.
    """
    return Server.query.filter(
        Server.active.is_(True),
        Server.group_id.in_(_group_ids(user)),
        ~Server.id.in_(_granted_server_ids(user)),
    )
